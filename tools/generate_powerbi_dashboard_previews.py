...
def industry_overview():
    ...
    by_line["oee_gap"] = by_line["oee"] - 0.85
    by_line["downtime_rate"] = by_line["downtime"] / by_line["planned"]
    by_line["defect_rate"] = by_line["defects"] / by_line["units"]
    units = production.groupby("line_name", as_index=False)["units_produced"].sum().sort_values("units_produced")
    lines_below_target = int((by_line["oee"] < 0.85).sum())
    fig = figure()
    ...
    add_card(fig, 4, f"{lines_below_target}", "Lines Below Target", "OEE < 85%", "bad" if lines_below_target else "good")
    ...
    ax2.axis("off")
    detail = by_line.merge(units, on="line_name")
    cell_text = [
        [r.line_name, f"{r.oee:.1%}", f"{r.oee_gap:+.1%}", f"{r.downtime_rate:.1%}", f"{r.defect_rate:.1%}", f"{r.units_produced:,.0f}"]
        for r in detail.itertuples()
    ]
    tbl = ax2.table(cellText=cell_text, colLabels=["Line", "OEE", "Gap", "Downtime", "Defects", "Units"], cellLoc="center", loc="center")
    ...
    for (row, col), cell in tbl.get_celld().items():
        ...
        elif cell_text[row - 1][0] == "Line 3" and col in [1, 2, 3]:
            cell.set_facecolor("#F6E9E3")
    ax2.set_title("Operating Detail", loc="left", fontsize=12, fontweight="bold", color=INK, pad=8)
...
def industry_oee_breakdown():
    ...
    top_reason = reason.sort_values("downtime_minutes", ascending=False).iloc[0]
    ...
    add_card(fig, 4, top_reason["downtime_reason"], "Top Downtime Reason")
    ...
    display["gap"] = display["oee"] - 85
    cell_text = [[r.line_name, r.area, f"{r.availability:.1f}%", f"{r.performance:.1f}%", f"{r.quality:.1f}%", f"{r.oee:.1f}%", f"{r.gap:+.1f}%"] for r in display.itertuples()]
    tbl = ax.table(cellText=cell_text, colLabels=["Line", "Area", "Availability", "Performance", "Quality", "OEE", "Gap"], cellLoc="center", loc="center")
    tbl.auto_set_font_size(False)
    tbl.set_fontsize(10)
    tbl.scale(1, 1.45)
    for (row, col), cell in tbl.get_celld().items():
        ...
        if row == 0:
            ...
            cell.set_facecolor("#F6E9E3" if col in [2, 5, 6] else WHITE)
        else:
...
def industry_cost():
    ...
    materials_share = by_type.loc[by_type["cost_type"].eq("Materials"), "amount"].sum() / total_cost
    highest_cost_type = by_type.iloc[0]["cost_type"]
    ...
    add_card(fig, 3, f"{materials_share:.1%}", "Materials Share")
    add_card(fig, 4, highest_cost_type, "Highest Cost Type")
...
def supply_overview():
    ...
    products_stocked_out = int(latest["stockout"].sum())
    fig = figure()
    add_header(fig, "Inventory Overview", "All Regions")
    add_card(fig, 1, f"${latest['inventory_value'].sum() / 1000:.1f}K", "Latest Inventory Value", f"week {latest_date:%Y-%m-%d}")
    add_card(fig, 2, f"{inventory['stockout'].mean():.1%}", "Stockout Rate")
    add_card(fig, 3, f"{orders['on_time'].mean():.1%}", "On-Time Orders")
    add_card(fig, 4, f"{products_stocked_out}", "Products Stocked Out", "latest week", "bad" if products_stocked_out else "good")
    ax = fig.add_axes(rect(24, 224, 1232, 232))
    ...
    ax1 = fig.add_axes(rect(24, 472, 608, 224))
    style(ax1, "Latest Inventory Value by Category")
...
def supply_stockouts():
    ...
    latest = inventory[inventory["snapshot_date"] == inventory["snapshot_date"].max()][["product_id", "stock_on_hand"]]
    stockout = stockout.merge(latest, on="product_id", how="left")
    top = stockout.sort_values("stockout_rate", ascending=False).head(10)
    fig = figure()
    ...
    add_card(fig, 4, f"{int(stockout['weeks'].sum()):,}", "Tracked Product-Weeks")
    ax = fig.add_axes(rect(24, 224, 1232, 232))
    style(ax, "Worst 10 Products by Stockout Rate")
    ...
    ax2.axis("off")
    display = top[["product_id", "product_name", "category", "weeks", "stockout_rate", "stock_on_hand"]].copy()
    display["stockout_rate"] = display["stockout_rate"].map(lambda v: f"{v:.1%}")
    tbl = ax2.table(cellText=display.values, colLabels=["Product ID", "Product", "Category", "Weeks", "Stockout Rate", "Latest Stock"], cellLoc="center", loc="center")
    tbl.auto_set_font_size(False)
    tbl.set_fontsize(8)
    tbl.scale(1, 1.05)
    for (row, col), cell in tbl.get_celld().items():
...
def supply_suppliers():
    ...
    perf = orders.dropna(subset=["supplier_name"]).groupby(["supplier_name", "region"], as_index=False).agg(
        ...
        late_orders=("on_time", lambda s: (~s).sum()),
    )
    ...
    add_card(fig, 4, f"{int((~orders['on_time']).sum()):,}", "Late Orders", status="bad")
    ...
    tbl = ax2.table(cellText=display[["supplier_name", "region", "total_orders", "late_orders", "on_time"]].values, colLabels=["Supplier", "Region", "Total Orders", "Late Orders", "On-Time %"], cellLoc="center", loc="center")
    tbl.auto_set_font_size(False)
    tbl.set_fontsize(8)
    tbl.scale(1, 1.08)
    for (row, col), cell in tbl.get_celld().items():
